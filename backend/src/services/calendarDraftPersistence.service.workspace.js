/**
 * Workspace copy: passes cycleIndex into ContentItem.create for per-cycle displayId.
 * Canonical file may be root-owned; keep in sync with calendarDraftPersistence.service.js if needed.
 */
...
async function persistCalendarDraft({ clientId, calendarDraft, createdBy }) {
  ...
  for (const item of items) {
    ...
    const cycleIndexRaw = item.cycleIndex;
    const cycleIndex =
      cycleIndexRaw !== undefined && cycleIndexRaw !== null && Number.isFinite(Number(cycleIndexRaw))
        ? Math.min(3, Math.max(1, Number(cycleIndexRaw)))
        : 1;
