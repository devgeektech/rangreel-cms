...
const { simulateSchedule } = require("./dragScheduleSimulation.service");
...
async function runManagerDragTask({
  ...
  // PROMPT 109 — Do not reject immediately: simulate borrow / schedule recovery first.
  if (sequenceViolation) {
    const tempTasks = normalizeDraftItemToDurationTasks(item);
    const movedTask = (tempTasks || []).find(
      (t) => String(t.role).toLowerCase() === String(role).toLowerCase()
    );
    const sim = await simulateSchedule(tempTasks, {
      movedTask: movedTask || { role, durationDays: schedulingMeta.finalDurationDays },
      newDate: resolvedTargetDate,
      durationPlan,
      pipelineKind,
      movedWorkflowRole: role,
    });

    if (sim.allowBorrow === true || sim.borrowingApplied === true) {
      schedulingMeta.allowBorrow = Boolean(sim.allowBorrow);
      schedulingMeta.borrowingApplied = Boolean(sim.borrowingApplied);
      schedulingMeta.p109Simulation = sim;
      if (sim.borrowingApplied) {
        schedulingMeta.borrowed = true;
        schedulingMeta.durationAdjusted = true;
      }
    } else {
      return {
        ...
        details: {
          ...
          from: sequenceViolation.aName,
          to: sequenceViolation.bName,
          fromDate: toYMD(sequenceViolation.aDate),
          toDate: toYMD(sequenceViolation.bDate),
        },
      };
    }
  }
  ...
  return {
    ...
    data: {
      ...
      /** PROMPT 109 — normalized duration tasks for this content item after drag */
      updatedTasks: Array.isArray(item.tasks) ? item.tasks : [],
    },
  };
  } catch (err) {
    if (session) {
