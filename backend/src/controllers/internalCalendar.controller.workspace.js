/**
 * Workspace copy: adds strategistAlias to GET payload.
 * Canonical file may be root-owned; keep in sync when changing internal calendar logic.
 */
...
const getInternalCalendar = async (req, res) => {
  try {
    ...
    const items = await ContentItem.find({ client: clientId })
      .select("title displayId strategistAlias taskType taskNumber type contentType clientPostingDate workflowStages isCustomCalendar weekendEnabled client")
